...
let $p : B \to \Omega$ denote the membership predicate so that $p(a) = \mathrm{tt} \iff a \in A$.
...
$p\iota = \mathrm{tt}!$. If $C$ is a set and $f : C \to B$ is a function such that $pf=\mathrm{tt}$, then there exists a unique function $\bar f : C \to {A}$ such that $\iota \bar f = f$, i.e.
...
> [!check]- Proof
> Clearly $p \iota = \mathrm{tt}!$ by construction.
> If $pf = \mathrm{tt}!$, then $\im f \sube A$ so we can take $\bar f(c) = f(c)$.
> <span class="QED"/>
